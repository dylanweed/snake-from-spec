#!/usr/bin/env node
// Builds every artifact directory (any top-level dir with a package.json
...
const links = artifacts.map((name) => `<li><a href="./${name}/">${name}</a></li>`).join("\n      ");
writeFileSync(
...
<html lang="en">
  ...
  <body>
    ...
    <p>Artifacts generated from the same specification:</p>
    <ul>
      ${links}
    </ul>
  </body>
...
console.log(`\nBuilt ${artifacts.length} artifact(s) into _site/`);
